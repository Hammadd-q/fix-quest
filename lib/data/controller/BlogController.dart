...
class BlogController extends BaseController {
  ...
  BlogController({required this.dioClient});

  Future<ApiResponse> blogs(context) async {
    showLoader(context);
    try {
      final response = await dioClient.get(AppConstants.blogs);
  ...
  Future<ApiResponse> blog(blogId, context) async {
    showLoader(context);
    try {
      final response = await dioClient.get(AppConstants.post.replaceFirst('{postId}', blogId.toString()));
      dismissLoader(context);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      dismissLoader(context);
