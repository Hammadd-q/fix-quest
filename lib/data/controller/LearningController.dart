...
class LearningController extends BaseController {
  ...
  LearningController({required this.dioClient});

  Future<ApiResponse> getAllLearning(context, userId) async {
    ...
    showLoader(context);
    try {
      var url = AppConstants.allLearning.replaceFirst("{id}", userId.toString());
  ...
  Future<ApiResponse> learningMedia(postId) async {
    try {
      final response = await dioClient.get(AppConstants.learningMedia.replaceFirst('{postId}', postId.toString()));
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
